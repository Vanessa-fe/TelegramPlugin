...
function resolvePublicBranding(metadata, status, planName) {
    ...
    const proBrandingEnabled = (status === client_1.PlatformSubscriptionStatus.ACTIVE ||
...
let LandingPagesService = class LandingPagesService {
    ...
    async getPublicPage(creatorSlug, pageSlug) {
        ...
        const branding = resolvePublicBranding(organization.metadata, organization.platformSubscription?.status, organization.platformSubscription?.platformPlan?.name);
