...
export function createBot(config) {
    ...
    bot.command("start", (ctx) => ctx.reply("👋 Bienvenue ! Votre accès sera géré automatiquement par la plateforme."));
    // Handle /buy command for Telegram Stars payments
    bot.command("buy", async (ctx) => {
        const args = ctx.match?.trim().split(' ');
        if (!args || args.length === 0 || args[0] === '') {
            await ctx.reply("❌ Usage: /buy <plan_id>\n\nExemple: /buy plan_abc123");
            return;
        }
        const planId = args[0];
        try {
            ...
            const displayName = [ctx.from.first_name, ctx.from.last_name]
                ...
                .join(" ")
                .trim();
            const invoice = await postApi("/payments/telegram-stars/invoice", {
                planId,
                customer: {
            ...
            await ctx.reply("❌ Erreur lors de la création de la facture. Veuillez réessayer plus tard.");
        }
    });
    ...
    bot.on("message:successful_payment", async (ctx) => {
        ...
        await ctx.reply("✅ Paiement reçu avec succès !\n\n" +
    ...
    bot.on("pre_checkout_query", async (ctx) => {
        ...
        try {
            ...
            const validation = await postApi("/payments/telegram-stars/validate-pre-checkout", { invoicePayload: query.invoice_payload });
            if (validation.valid) {
                ...
                await ctx.answerPreCheckoutQuery(false, validation.error || "Ce produit n'est plus disponible");
            }
        }
        catch (error) {
