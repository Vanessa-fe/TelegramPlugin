...
function loadEnv() {
    ...
    const apiBaseUrl = parsed.data.DISCORD_API_URL ??
...
export function createBot(config) {
    ...
    async function handleMessageCreate(message) {
        ...
        try {
            ...
            if (!hasRequiredPermissions(guild)) {
                ...
                await message.reply("❌ Je n'ai pas les permissions nécessaires (Gérer les rôles). " +
            ...
            const result = await postApi("/channels/verification/discord/verify", {
                code,
            ...
            if (result.success) {
                ...
                if (message.channel.isSendable()) {
                    await message.channel.send("✅ **Serveur vérifié avec succès !**\n\n" +
                ...
                await message.reply(`❌ Échec de la vérification: ${result.message || "Code invalide ou expiré"}`);
            }
        }
        catch (error) {
    ...
    return {
        ...
        async grantAccessToUser(guildId, roleId, discordUserId) {
            try {
        ...
        async revokeAccessFromUser(guildId, roleId, discordUserId) {
            try {
                const guild = await client.guilds.fetch(guildId);
                const role = await guild.roles.fetch(roleId);
                if (!role) {
